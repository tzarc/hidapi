...
static PHIDP_PREPARSED_DATA alloc_preparsed_data_from_file(char* filename)
{
	FILE* file;
	...
	PHIDP_PREPARSED_DATA pp_data = &static_pp_data;
...
int main(int argc, char* argv[])
{
	...
	PHIDP_PREPARSED_DATA pp_data = alloc_preparsed_data_from_file(argv[1]);
	if (pp_data == NULL) {
		return EXIT_FAILURE;
	}
	...
	int res = rd_reconstructor(pp_data, report_descriptor, sizeof(report_descriptor), &err_str);

	if (res < 0) {
		result = EXIT_FAILURE;
		if (err_str) {
			fprintf(stderr, "Reconstruct err: %ls\n", err_str);
		}
		goto end;
	}
